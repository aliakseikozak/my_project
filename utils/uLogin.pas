...
type
  ...
  private
    ...
    property  AdvVisible: Boolean read FAdvVisible write SetAdvVisible;  // дополнительная панель - видимая или нет
...
begin
  ...
  SetHeight;
  if FAdvVisible then
    btAdvance.Caption:='<< Скрыть'
  else
    btAdvance.Caption:='Дополнительно >>';
  if Self.Active then
   if FAdvVisible then
     edHost.setFocus
   else
