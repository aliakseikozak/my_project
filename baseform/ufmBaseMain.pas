...
type
  ...
  private
    ...
    // Передача для всех форм OnResize
    procedure OnResizeAllForms;
    // Изменить размер  TabSheet  PageControlForms (меню)
  ...
  public
    ...
    // проверяет права пользователей на работу с модулями. гасит не доступные меню
    procedure MainMenuTest; virtual;
    // добавляет форму в список форм на главном с=меню приложения
    procedure ListFormsADD(Form: TForm);
    // удаляет  форму из списка форм на главном с=меню приложения
    procedure ListFormsDel(Form: TForm);
    // Передаем для формы во вкладке OnResize
    procedure ResizeForm(ThePanel: TcxTabSheet);
    // Передаем на форму WM_ACTIVATE
...
begin
  ...
  begin
    // есть параметры запуска - пробуем с ними законнектится
...
begin
  if dsBase.DataSet.FieldByName('is_null').AsBoolean = False then
    if not OpenForm(dsBase.DataSet.FieldByName('form_unit').AsString, dsBase.DataSet.FieldByName('form_class').AsString, dsBase.DataSet.FieldByName('form_name').AsString,
      ...
      DialogStop('Не найден класс формы: ' + dsBase.DataSet.FieldByName('form_comment').AsString);
end;
...
begin
  inherited;
  if cxSearch.Text='Текст для поиска...' then
    cxSearch.Text:='';
end;
...
begin
  inherited;
  if cxSearch.Text<>'Текст для поиска...' then cxSearch.Text:='Текст для поиска...';
end;

procedure TfmBaseMain.Filtered();
begin
  if (length(cxSearch.Text)>2) and (cxSearch.Text<>'Текст для поиска...') then
   begin
     try
...
begin
  ...
  self.Caption := Application.Title + ' Сервер ' + gServerName + ' Build ' + Resource.GetBuildInfoAsString(ParamStr(0)) + '. Пользователь: ' +
    Resource.sessionInfo.UserName;
  if not LoadDBTreeListMainMenu then
    DialogStop('Не удалось загрузить меню приложения');
...
begin
  // Изменение размера для всех подчененных форм
  OnResizeAllForms;
end;
...
begin
  ...
  begin
    ...
    dxbrbtn1.Caption := 'Уведомления: ' + IntToStr(i);
  end
  else
  begin
...
begin
  if DialogQuest('Завершить работу с модулем?', 'Завершение работы') then
  begin
    CanClose := True;
    inherited;
  end
...
begin
  ...
  ProgressBar.Properties.Text := 'Завершение работы';
...
// Обновление общих справочников
procedure TfmBaseMain.ActUpdateSpravExecute(Sender: TObject);
var
  cr: TCursor;
  AWorkCountMax: Int64; // Количество шагов прогресс-бара
  LastWorkCount: Int64; // Текущий шаг прогресс-бара
  tempTDBData: TDBData; // временная переменная для TDBData
  FContext: TRttiContext;
  FType: TRttiType;
  FProp: TRttiProperty;
  FField: TRttiField;
...
begin
  inherited;
  OpenForm('uSQLLog', 'TfrmSQLQueryLog', 'frmSQLQueryLog', 'Лог запросов');
...
begin
  ...
  try
    ...
    // Удалим файл из темпа перед распаковкой если он там есть
  ...
  // Проверим наличие распакованного файла
  if not FileExists(ExtractFilePath(Application.ExeName) + 'Temp\' + ZipedFileName) then
    Exit;

  // Попытка переименовать FileName в FileName.bak
...
begin
  try
    ...
    begin
      {$IFDEF DEBUG}
       if DialogQuest('Доступна новая версия программы. Обновить?') then
         begin
    ...
    begin
      if (not fIsAutoUpdate) then
        DialogInfo('Обновление приложение не требуется.');
    end;
  finally
    FreeAndNil(Version);
  end;
