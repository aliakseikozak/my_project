...
procedure TfmMedinspector.actDelExecute(Sender: TObject);
begin

  if DialogQuest('Удалить запись? Вы уверены?', 'Удаление записи') then
  begin
    dbIU := DataSetQuery('medinspector_del', [dbBase.DataSet.FieldByName('id').Value, Resource.sessionInfo.token_id]);
    actGetData.Execute;
  end;
...
procedure TfmMedinspector.btnSelectClick(Sender: TObject);
begin
  actGetData.Execute;
...
procedure TfmMedinspector.grBaseDBTableViewCellDblClick(
