inherited fmOffenses: TfmOffenses
  Caption = #1044#1072#1085#1085#1099#1077' '#1086#1073' '#1072#1076#1084#1080#1085#1080#1089#1090#1088#1072#1090#1080#1074#1085#1099#1077' '#1087#1088#1072#1074#1086#1085#1072#1088#1091#1096#1077#1085#1080#1103#1093
  ClientWidth = 990
  ExplicitWidth = 1006
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnParam: TPanel
    Width = 990
    TabOrder = 2
    ExplicitWidth = 990
    inherited lbParam: TcxLabel
      Style.IsFontAssigned = True
      ExplicitWidth = 988
    end
  end
  inherited pnSelect: TPanel
    Width = 990
    TabOrder = 4
    ExplicitWidth = 990
    inherited dxLCPnlSelect: TdxLayoutControl
      Left = 783
      ExplicitLeft = 783
      inherited dxLCPnlSelectGroup_Root: TdxLayoutGroup
        Index = -1
      end
      inherited dxLIbtnCancel: TdxLayoutItem
        Index = 1
      end
      inherited dxLIbtnSelect: TdxLayoutItem
        Index = 0
      end
    end
  end
  inherited gbGridBase: TPanel
    Width = 990
    ExplicitWidth = 990
    inherited grBase: TcxGrid
      Width = 988
      ExplicitWidth = 988
      inherited grBaseDBTableView: TcxGridDBTableView
        OnCellDblClick = grBaseDBTableViewCellDblClick
        object grBaseDBTableViewColumn10: TcxGridDBColumn
          Caption = #1050#1086#1076' '#1079#1072#1087#1080#1089#1080
          DataBinding.FieldName = 'id'
        end
        object grBaseDBTableViewColumn1: TcxGridDBColumn
          Caption = #1044#1072#1090#1072' '#1089#1086#1074#1077#1088#1096#1077#1085#1080#1103' '#1087#1088#1072#1074#1086#1085#1072#1088#1091#1096#1077#1085#1080#1103
          DataBinding.FieldName = 'offenses_date'
          Width = 110
        end
        object grBaseDBTableViewColumn2: TcxGridDBColumn
          Caption = #1044#1072#1090#1072' '#1087#1088#1080#1074#1083#1077#1095#1077#1085#1080#1103' '#1082' '#1072#1076#1084'.'#1086#1090#1074#1077#1090#1089#1090#1074#1077#1085#1085#1086#1089#1090#1080
          DataBinding.FieldName = 'attraction_date'
          Width = 146
        end
        object grBaseDBTableViewColumn3: TcxGridDBColumn
          Caption = #1056#1077#1075'.'#1085#1086#1084#1077#1088' '#1040#1055' '#1074' '#1045#1043#1041#1044#1055
          DataBinding.FieldName = 'reg_num'
          Width = 96
        end
        object grBaseDBTableViewColumn4: TcxGridDBColumn
          Caption = #1050#1077#1084' '#1087#1088#1080#1074#1083#1077#1082#1072#1083#1089#1103
          DataBinding.FieldName = 'name_org'
          Width = 119
        end
        object grBaseDBTableViewColumn5: TcxGridDBColumn
          Caption = #1057#1090#1072#1090#1100#1103' '#1050#1086#1040#1055
          DataBinding.FieldName = 'name_type'
          Width = 115
        end
        object grBaseDBTableViewColumn6: TcxGridDBColumn
          Caption = #1042#1080#1076' '#1087#1088#1072#1074#1086#1085#1072#1088#1091#1096#1077#1085#1080#1103
          DataBinding.FieldName = 'state_support_id'
          Width = 127
        end
        object grBaseDBTableViewColumn7: TcxGridDBColumn
          Caption = #1044#1072#1090#1072' '#1089#1086#1079#1076#1072#1085#1080#1103' '#1079#1072#1087#1080#1089#1080
          DataBinding.FieldName = 'create_date'
          Width = 110
        end
        object grBaseDBTableViewColumn8: TcxGridDBColumn
          Caption = #1044#1072#1090#1072' '#1086#1073#1085#1086#1074#1083#1077#1085#1080#1103' '#1079#1072#1087#1080#1089#1080
          DataBinding.FieldName = 'change_date'
          Width = 115
        end
        object grBaseDBTableViewColumn9: TcxGridDBColumn
          Caption = #1055#1086#1083#1100#1079#1086#1074#1072#1090#1077#1083#1100
          DataBinding.FieldName = 'username'
        end
      end
    end
  end
  inherited cxspltrParam: TcxSplitter
    Width = 990
    ExplicitWidth = 990
  end
  inherited BarManager: TdxBarManager
    DockControlHeights = (
      0
      0
      24
      0)
  end
  inherited ActionList: TActionList
    inherited actGetData: TAction
      OnExecute = actGetDataExecute
    end
  end
end
